...
APP_NAME = "MINIBUS Lite v0.4.0"
REFRESH_SECONDS = 5
LEARN_TIMEOUT_SECONDS = 25
CONFIG_DIR = Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "minibus"
...
#
# On Windows and macOS MINIBUS Lite v0.4.0 provides discovery and
...
def connect_audio_ports(source: str, dest: str) -> tuple[bool, str]:
    ...
    On PipeWire systems this wraps ``connect_ports`` directly. On other
    platforms we offer limited virtual connection support. If either the
    source or destination contains a keyword from ``VIRTUAL_DEVICE_KEYWORDS``
    (e.g. "CABLE", "VB", "Voicemeeter", "BlackHole", or "Loopback"), the
    connection is stored in the ``_virtual_connections`` set and treated as
    successful. This mirrors the common scenario where a user relies on a
    virtual audio device for routing. For all other non-PipeWire routes the
    function returns a message explaining that full patching is not yet
    implemented.
    """
    # Normal Linux behaviour delegates to pw-link via connect_ports.
    if audio_backend_name() == "PipeWire":
        return connect_ports(source, dest)
    # Ensure both ports are chosen.
    if not source or not dest:
        return False, "Choose both a source and a destination."
    # For non-PipeWire backends attempt a virtual connection when either port
    # references a known virtual device keyword.
    s_low = source.lower()
    d_low = dest.lower()
    if any(k in s_low for k in VIRTUAL_DEVICE_KEYWORDS) or any(k in d_low for k in VIRTUAL_DEVICE_KEYWORDS):
        key = (source, dest)
        if key in _virtual_connections:
            return True, "Already connected (virtual)"
        _virtual_connections.add(key)
        return True, "Connected (virtual)"
    # Otherwise report that app-to-app patching is not available.
    return False, (
        f"{audio_backend_name()} discovery is available, but app-to-app patching is not implemented in this Lite backend yet. "
        "Use Linux/PipeWire for full routing, or add a platform routing adapter/virtual audio driver."
    )
...
def disconnect_audio_ports(source: str, dest: str) -> tuple[bool, str]:
    ...
    On PipeWire systems this wraps ``disconnect_ports`` directly. For non-
    PipeWire backends, if the source/destination pair exists in the
    ``_virtual_connections`` set it is removed and a success message is
    returned. Otherwise a helpful message is provided.
    """
    if audio_backend_name() == "PipeWire":
    ...
    if not source or not dest:
        return False, "Choose both a source and a destination."
    key = (source, dest)
    # Remove the connection if present.
    if key in _virtual_connections:
        _virtual_connections.discard(key)
        return True, "Disconnected"
    return False, f"{audio_backend_name()} link removal is not implemented in this Lite backend yet."


def current_links() -> list[str]:
    """Return a list of active links.

    For PipeWire this proxies the ``pw-link -l`` output. For non-PipeWire
    backends the returned list reflects any virtual connections created via
    ``connect_audio_ports``. If no virtual connections are present the
    function falls back to a generic message indicating that the graph is
    unavailable.
    """
    if audio_backend_name() != "PipeWire":
        if _virtual_connections:
            return [f"{src} → {dst}" for (src, dst) in sorted(_virtual_connections)]
        return [f"{audio_backend_name()} link graph is not available in MINIBUS Lite yet."]
...
def collect_diagnostics() -> list[tuple[str, str]]:
    ...
    if backend == "PipeWire":
        ...
        add("INFO", "WASAPI backend is discovery-only in this Lite release.")
        add("INFO", "Full Windows patching needs a routing adapter and usually a virtual audio device.")
        shell = shutil.which("powershell") or shutil.which("pwsh")
        add("PASS" if shell else "WARN", f"PowerShell {'found at ' + shell if shell else 'not found'}")
    elif backend == "CoreAudio":
        add("INFO", "CoreAudio backend is discovery-only in this Lite release.")
        add("INFO", "Full macOS patching needs a CoreAudio routing adapter and usually a virtual driver such as BlackHole.")
    ...
    if backend_supports_patching():
        add("PASS", f"{backend} patching enabled")
    else:
...
class Lane:
    ...
    def disconnect_saved_link(self) -> bool:
        if not self.patched_source or not self.patched_dest:
            return False
        ok, _msg = disconnect_ports(self.patched_source, self.patched_dest)
        if ok:
            self.connected = False
            self.app.unregister_lane_link(self)
        return ok
    ...
    def reconnect_saved_link(self) -> bool:
        ...
        if not allowed:
            return False
        ok, _msg = connect_ports(self.patched_source, self.patched_dest)
        if ok:
            self.connected = True
            self.app.register_lane_link(self)
        return ok
...
class MiniBusApp:
    ...
    def create_bus_dialog(self):
        if audio_backend_name() != "PipeWire":
            messagebox.showinfo(
                ...
                f"{audio_backend_name()} virtual bus creation is not implemented in MINIBUS Lite yet. "
                "On Windows/macOS, use a virtual audio driver such as VB-CABLE/Voicemeeter or BlackHole and a future backend adapter."
            )
            return
    ...
    def show_links(self):
        ...
        win.title("Current PipeWire links")
        win.geometry("760x420")
        txt = tk.Text(win, wrap="none")
        txt.pack(fill="both", expand=True)
        txt.insert("1.0", "\n".join(links) if links else "No links returned by pw-link -l")
